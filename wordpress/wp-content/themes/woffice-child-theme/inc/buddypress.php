	...
	function woffice_members_filter() {
        ...
        <div id="woffice-roles-filter" class="dropdown text-center">
            ...
            <div class="dropdown-menu" role="menu">
                ...
                <a href="javascript:void(0)" data-role="0"
                   class="dropdown-item"><?php esc_html_e( 'All members', 'woffice' ); ?></a>

                <div class="dropdown-divider"></div>

				<?php foreach ( $wp_roles->roles as $key => $value ): ?>
					<?php if ( substr( $key, 0, 4 ) != 'bbp_' && !in_array( $key, $buddy_excluded_directory ) && array_key_exists( $key, $count_users['avail_roles'] ) && ( $count_users['avail_roles'][$key] > 0 ) ) : ?>
                        <a href="javascript:void(0)" data-role="<?php echo esc_attr( $key ); ?>" class="dropdown-item">
...
if ( !function_exists( 'woffice_render_advanced_search_fields' ) ) {
	...
	function woffice_render_advanced_search_fields( $render_form = FALSE ) {
		...
		if ( !woffice_bp_is_active( 'xprofile' ) ) return;

		if ( !bp_has_profile( array( 'user_id' => 0, 'fetch_field_data' => FALSE ) ) ) return;

		$style = ( isset( $_POST['advanced-search-submit'] ) || $render_form ) ? 'style="display: block;"' : '';
		echo '<div id="woffice-members-advanced-search"  class="bg-light p-4 mb-4 rounded" ' . $style . '>';
		...
		if ( !empty( $form_title ) ) echo '<h3 class="mt-0">' . $form_title . '</h3>';

		if ( $render_form ) echo '<form id="woffice-members-advanced-search-form" action="' . bp_get_members_directory_permalink() . '" method="POST">';
	...
	function woffice_get_advanced_search_fields_from_post_request() {
		...
		while ( bp_profile_groups() ) {
			...
			while ( bp_profile_fields() ) {
				...
				if ( empty( $_POST[$field_input_name] ) && $field_type !== 'number' ) continue;
	...
	function woffice_exclude_members( $roles, $direction ) {
		...
		if ( is_array( $roles ) ) {
			...
			foreach ( $roles as $role ) {
				$requested_users_role = get_users( array(
					'role'    => $role,
					'fields'  => 'id',
...
if ( !function_exists( 'woffice_get_cover_image' ) ) {
	...
	function woffice_get_cover_image( $user_ID ) {
		...
		$the_cover_from_extension = ( woffice_bp_is_active( 'xprofile' ) ) ? bp_get_profile_field_data( array(
			'field'   => 'woffice_cover',
			'user_id' => $user_ID
		) ) : '';
		...
		$the_cover_old = ( woffice_bp_is_active( 'xprofile' ) ) ? bp_get_profile_field_data( array(
			'field'   => 'Cover',
			'user_id' => $user_ID
		) ) : '';
		$array         = array();
...
if ( !function_exists( 'woffice_user_notifications' ) ) {
	...
	function woffice_user_notifications( $component ) {
		...
		} else if ( $component == "messages" && woffice_bp_is_active( "messages" ) ) {
...
if ( !function_exists( 'woffice_calculate_time_span' ) ) {
	...
	function woffice_calculate_time_span( $date ) {
		...
		if ( $seconds < 60 ) $time = $secs . __( " seconds ago", "woffice" ); else if ( $seconds < 60 * 60 ) $time = $mins . __( " min ago", "woffice" ); else if ( $seconds < 24 * 60 * 60 ) $time = $hours . __( " hours ago", "woffice" ); else if ( $seconds < 24 * 60 * 60 ) $time = $day . __( " day ago", "woffice" ); else
...
if ( !function_exists( 'wofficeNoticationsGetHandler' ) ) {
	...
	function wofficeNoticationsGetHandler() {
		...
		if ( !function_exists( 'bp_notifications_get_unread_notification_count' ) || !function_exists( 'bp_notifications_get_notifications_for_user' ) ) return;
            ...
            */
			if ( !empty( $notifications ) ) {
				...
				foreach ( $notifications as $notification ) {
					...
					if ( ( $notification->component_name == 'woffice_wiki' || $notification->component_name == 'woffice_project' || $notification->component_name == 'woffice_blog' ) && ( substr( $notification->content, 0, 4 ) == 'Your' ) || $notification->component_name == 'woffice_project' && ( $notification->component_action == 'woffice_project_assigned_todo' || $notification->component_action == 'woffice_project_assigned_member' ) && ( substr( $notification->content, 0, 4 ) != 'You ' ) && $notification->secondary_item_id != 0 ) {
						...
						$woffice_wp_users = get_users( array(
					...
					echo '<i class="fa component-icon ' . $icon_class . '"></i> ' . $notification->content . ' <span class="notification-freshness">(' . $time_difference . ')</span>';
					echo '</a>';
...
if ( !function_exists( 'woffice_user_sidebar' ) ) {
	...
	function woffice_user_sidebar() {
		...
		echo '<span id="closer-usersidebar" class="close-usersidebar"><i class="fa fa-close"></i></span>';
		echo '<div class="sidebar-wrapper"><nav>';
		...
		if ( class_exists( 'bbPress' ) ) {
			$forum_li = '<li id="forums-li" class="menu-parent">
                    <a href="' . $profile . 'forums/">' . __( 'Forums', 'woffice' ) . '</a>
                    <ul class="sub-menu">
                        <li id="general-personal-li" class="menu-child">
                            <a href="' . $profile . 'forums/">' . __( 'Forums', 'woffice' ) . '</a>
                        </li>
                        <li id="general-personal-li" class="menu-child">
                            <a href="' . $profile . 'forums/replies/">' . __( 'Replies', 'woffice' ) . '</a>
                        </li>
                        <li id="general-personal-li" class="menu-child">
                            <a href="' . $profile . 'forums/engagements/">' . __( 'Engagements', 'woffice' ) . '</a>
                        </li>
                        <li id="general-personal-li" class="menu-child">
                            <a href="' . $profile . 'forums/favorites/">' . __( 'fFvorites', 'woffice' ) . '</a>
                        </li>
                        <li id="general-personal-li" class="menu-child">
                            <a href="' . $profile . 'forums/subscriptions/">' . __( 'Subscriptions', 'woffice' ) . '</a>
                        </li>
                    </ul>
                </li>';

			//echo apply_filters( 'woffice_forum_li', $forum_li, $profile );
		}

		if ( class_exists( 'RTMedia' ) ) {
			$rtmedia_li = '<li id="rtmedia-li" class="menu-parent">
                    <a href="' . $profile . 'media/">' . __( 'Media', 'woffice' ) . '</a>
                    <ul class="sub-menu">
                        <li id="general-personal-li" class="menu-child">
                            <a href="' . $profile . 'media/">' . __( 'Media', 'woffice' ) . '</a>
                        </li>
                        <li id="general-personal-li" class="menu-child">
                            <a href="' . $profile . 'media/album/">' . __( 'Album', 'woffice' ) . '</a>
                        </li>
                        <li id="general-personal-li" class="menu-child">
                            <a href="' . $profile . 'media/photo/">' . __( 'Photo', 'woffice' ) . '</a>
                        </li>
                        <li id="general-personal-li" class="menu-child">
                            <a href="' . $profile . 'media/video/">' . __( 'Video', 'woffice' ) . '</a>
                        </li>
                        <li id="general-personal-li" class="menu-child">
                            <a href="' . $profile . 'media/music/">' . __( 'Music', 'woffice' ) . '</a>
                        </li>
                    </ul>
                </li>';

			//echo apply_filters( 'woffice_rtmedia_li', $rtmedia_li, $profile );
		}
		...
		if ( has_nav_menu( 'woffice_user' ) ) {
			wp_nav_menu( array(
				'theme_location' => 'woffice_user',
				'menu_id'        => 'dropdown-user-menu',
				'container'      => ''
			) );
		}
...
add_action( 'bp_after_member_header', 'woffice_member_social_extend' );
...
if ( !function_exists( 'bp_woffice_directory_members_search_form' ) ) {
	...
	function bp_woffice_directory_members_search_form() {
        ...
        <form action="" method="get" id="search-members-form">
            <label><input type="text" name="s" id="members_search"
                          placeholder="<?php echo esc_attr( $search_value ) ?>"/></label>
...
if ( !function_exists( 'woffice_list_xprofile_fields' ) ) {
	...
	function woffice_list_xprofile_fields( $user_ID, $is_printable = TRUE ) {
		...
		foreach ( $fields_values as $field_name => $field ) {
			...
			if ( $field_name == 'user_login' || $field_name == 'user_nicename' || $field_name == 'user_email' ) continue;

			// Skip display name used by BuddyPress
			if ( $field['field_id'] == 1 && !apply_filters( 'woffice_include_display_name_in_members_loop_fields', FALSE ) ) continue;
		...
		if ( !empty( $formatted_social_items ) ) {
			$html .= '<div class="member-xprofile-social-items mt-2 mb-2">';
...
if ( !function_exists( 'woffice_profile_note_content' ) ) {
	...
	function woffice_profile_note_content() {
        ...
        <form action="<?php echo esc_attr( $protocol ) . '://' . woffice_get_http_host() . woffice_get_request_uri(); ?>"
              method="POST" enctype="multipart/form-data" id="woffice_user_notes">
			<?php wp_editor( $the_notes, 'woffice_notes', array(
				'media_buttons' => FALSE,
				'textarea_name' => 'woffice_notes'
			) ); ?>
            <button type="submit" class="btn btn-default"><i
                        class="fa fa-edit"></i> <?php _e( 'Save my notes', 'woffice' ); ?></button>
        </form>
	...
	function woffice_is_user_allowed_buddypress( $type ) {
		...
		if ( $type == "view" ) {
			...
			if ( ( bp_is_members_component() || bp_is_user() ) && $buddy_members_state == "private" && !is_user_logged_in() ) {
                ...
                else if ( woffice_bp_is_active( 'activity' ) && bp_is_activity_component() && $buddy_activity_state == "private" && !is_user_logged_in() ) {
					//return apply_filters('woffice_is_user_allowed_buddypress', false);
					$is_allowed = FALSE;
				} else {
					//return apply_filters('woffice_is_user_allowed_buddypress', true);
	...
	function woffice_members_filter_query( $query_string ) {
		...
		if ( isset( $matching_field_id, $user_id ) ) {
			...
			if ( isset( $matching_field->data->value ) ) {
				...
				$all_include_string = ( $all_include_ids !== '' && sizeof( $all_include_ids ) > 0 ) ? implode( ',', $all_include_ids ) : '';
			}
		}
...
if ( !function_exists( 'woffice_custom_bp_alerts' ) ) {
	function woffice_custom_bp_alerts() {
		...
		if ( $message == "Notice successfully created." ) return;
...
if ( !function_exists( 'woffice_render_notices' ) ) {
	...
	function woffice_render_notices() {
		...
		if ( !woffice_bp_is_active( 'messages' ) || !is_user_logged_in() ) return;

		$user_id = get_current_user_id();
		$notice  = BP_Messages_Notice::get_active();

		// If no notice
		if ( ( empty( $notice ) ) || !isset( $notice->id ) || ( $notice->id === NULL ) ) return;
		...
		if ( !is_array( $closed_notices ) ) $closed_notices = array();

		// We checked that our current active notices hasn't been closed by the logged user
		if ( in_array( $notice->id, $closed_notices ) ) return;
		...
		if ( !is_array( $notice_ids ) ) $notice_ids = array();
	...
	function woffice_order_members_options( $filter ) {
		...
		if ( !empty( $filter ) && isset( $buddy_sort_members_by ) && isset( $filter[$buddy_sort_members_by] ) ) {
			...
			foreach ( $filter as $k => $v ) {
				if ( $k === $buddy_sort_members_by ) continue;
...
add_action( 'bp_before_member_header_meta', 'woffice_add_item_in_profile' );
