...
if(!function_exists('woffice_title')) {
    ...
    function woffice_title($title) {
		...
		if (empty($final_image)) {
			$title_side_image = woffice_get_settings_option('title_side_image');
			$title_side_image = (empty($title_side_image)) ? get_template_directory_uri() . '/images/bannerBG.png' : $title_side_image;
			$title_side_image = (is_array($title_side_image)) ? $title_side_image : array('url' => $title_side_image);

			if (!empty( $title_side_image ) && array_key_exists('url', $title_side_image)) {
				$final_side_image = $title_side_image['url'];
			}
		}
        ...
        if (empty($slider_featured) || (!shortcode_exists('rev_slider') && !class_exists('FW_Extension_Slider'))) {
	        ...
	        if (empty($final_image)) {
		        ...
		        $main_featured_image = (empty($main_featured_image)) ? get_template_directory_uri() . '/images/login-bg.png' : $main_featured_image;
...
if(!function_exists('woffice_paging_nav')) {
    ...
    function woffice_paging_nav($custom_query = null)
    {
        ...
        echo '<div class="blog-next-page text-right">' . "\n";
...
function woffice_main_content_classes(){
    ...
    } else if($left_bar !== 'has-navigation-hidden' && $right_bar !== 'has-sidebar-hidden' || $sidebar_state === 'show' && $menu_default !== 'close' ){
        $class = "col-md-6 col-lg-8 222";
    } else if($left_bar === 'has-navigation-hidden' && $right_bar !== 'has-sidebar-hidden' || $left_bar !== 'has-navigation-hidden' && $right_bar === 'has-sidebar-hidden'){
        $class = "col-md-6 col-lg-10 333";
    }
